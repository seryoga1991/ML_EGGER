...
def prepare_structs_for_comparisson(debitor_dict,debitor,filter_dict,wordlist):
        ...
        filtered_dict = data_manipulation_helper.filter_sdDicts(filter_dict,'headers','SOLD_TO',debitor,wordlist)
        docs_to_compare = [filtered_dict['headers'][cfg.sd_key_val_pair['headers']],pandas.Series(exemplary_non_spam_doc),pandas.Series([]),pandas.Series([])]
        wl = wordlist[wordlist['DOC_NUMBER'].isin(docs_to_compare[0])]
...
def determine_all_spam(classified_dict):
    wl = data_loader.load_wordlists()
    sd = data_loader.SapData(cfg.path_to_sap_data,data_loader.tangro_om,wl, 0.)
    spam_list = []
    non_spam_list = []
    cfg.name_appendix = 'temp_corr'
    for key in classified_dict:
        docs_to_compare,wordlist = prepare_structs_for_comparisson(debitor_dict = classified_dict, debitor = key, filter_dict = sd.__train_dict, wordlist = wl)
        file = correlacte_docs(wordlist,docs_to_compare)
        temp_spam_list , temp_non_spam_list = data_manipulation_helper.get_spam_by_correlation_scores(file,0.8)
