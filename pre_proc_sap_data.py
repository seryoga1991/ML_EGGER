...
def __get_column_numb_to_assign(path_to_file, all_columns, tangro_modul):
    column_pos = __determine_col_pos_from_header(
        path_to_file, all_columns, tangro_modul)
    return column_pos
...
def __determine_col_pos_from_header(path_to_file,all_columns, tangro_list):
    for item in tangro_list:
        if item[0] in path_to_file:
            column_pos = all_columns.index(item[1]) + 1
            break
...
def get_column_specifics(file,path_to_file,tangro_modul):
    ...
    all_columns = file.columns.values.tolist()
    assigned_column_numb = __get_column_numb_to_assign(
        path_to_file, all_columns , tangro_modul)
    total_column_number_original = len(all_columns)
...
def preproc_sap_data(path_to_file, tangro_modul,separator):
    try:
        file = pd.read_csv(path_to_file, sep=',')
        return file
    except pd.errors.ParserError:
